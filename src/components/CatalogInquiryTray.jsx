...
const trayVariants = {
  hidden: { y: '100%' },
  visible: { y: 0, transition: SPRING.tray },
  exit: { y: '100%', transition: SPRING.trayClose },
}

const itemVariants = {
  hidden: { opacity: 0, y: 12 },
  visible: { opacity: 1, y: 0, transition: SPRING.item },
}

const persistentBarVariants = {
  hidden: { y: 20, opacity: 0 },
  visible: { y: 0, opacity: 1, transition: SPRING.tray },
...
export default function CatalogInquiryTray({
  ...
  // Actual overflow detection via ResizeObserver
  useEffect(() => {
    const el = scrollRef.current
    if (!el || !expanded) return
    const check = () => setHasScrollableContent(el.scrollHeight > el.clientHeight)
    check()
    const ro = new ResizeObserver(check)
    ro.observe(el)
    return () => ro.disconnect()
  }, [safeItems.length, expanded])
          ...
          >
            <div className="flex flex-col" style={{ maxHeight: '50vh' }}>
              ...
              <div className="shrink-0 relative flex items-center justify-between px-4 pt-3 pb-2">
                ...
                {hasScrollableContent && showScrollCue && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    ...
                    >
                      <ChevronDown className="h-4 w-4" style={{ color: COLOR.goldPrimary }} />
                    </motion.div>
                  </div>
                )}
              ...
              <div className="relative flex flex-col flex-1 min-h-0 overflow-hidden">
                ...
                {hasScrollableContent && safeItems.length > 6 && (
                  <div
                    className="pointer-events-none absolute right-2 top-1/2 z-10 flex -translate-y-1/2 flex-col items-center"
                    aria-hidden="true"
                  >
                    <div
                    ...
                    >
                      <div
                        ...
                        style={{
                          height: `${Math.max(4, (1 / safeItems.length) * 40)}px`,
                          top: `${scrollProgress * (40 - Math.max(4, (1 / safeItems.length) * 40))}px`,
      ...
      <AnimatePresence>
        {!expanded && (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="fixed inset-x-0 bottom-4 z-50 mx-auto w-[calc(100%-2rem)] max-w-md rounded-full border px-4 py-3"
            style={{
